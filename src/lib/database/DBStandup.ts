...
export class DBStandup {
	...
	async standupAdd(team_id: number, user_id: number, message_id: number, standup_content: string) {
		try {
			await this.db.$connect();

			const query_ensure_team_exists = new Sql([`INSERT INTO standup_teams (id) VALUES ($1) ON CONFLICT (id) DO NOTHING;`], [team_id]);
