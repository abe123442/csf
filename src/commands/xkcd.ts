...
export class XkcdCommand extends Command {
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addSubcommand((subcommand) => subcommand.setName('latest').setDescription('Get the latest xkcd comic.'))
				.addSubcommand((subcommand) =>
					subcommand
						.setName('get')
						.setDescription('Get xkcd comic by its id.')
						.addIntegerOption((option) =>
							option.setName('comic-id').setRequired(true).setDescription('The number id of the xkcd comic you want to get')
						)
				)
				.addSubcommand((subcommand) => subcommand.setName('random').setDescription('Get a random xkcd comic.'))
		);
	}
	...
	private async xkcd_response(err: any, res: IXkcdJSON, interaction: Command.ChatInputCommandInteraction) {
		if (err) {
			...
			const embed = new EmbedBuilder().setTitle(res.safe_title).setImage(res.img);
