...
export class AnonymousPostCommand extends Command {
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) => {
			builder
				...
				.addSubcommand((subcommand) =>
					subcommand
						...
						.addStringOption((option) =>
							option.setName('message').setDescription('Enter the text you wish to post anonymously').setRequired(true)
						)
				)
				.addSubcommand((subcommand) =>
					subcommand
						...
						.addChannelOption((option) =>
							option.setName('channel').setDescription('Enter the channel you wish to post anonymously in').setRequired(true)
						)
				)
				.addSubcommand((subcommand) =>
					subcommand
				...
				.addSubcommand((subcommand) =>
					subcommand.setName('allowcurrent').setDescription('[ADMIN] Allows channel which command is executed to be added.')
				)
				.addSubcommand((subcommand) =>
					subcommand
						.setName('disallow')
				...
				.addSubcommand((subcommand) =>
					subcommand.setName('disallowcurrent').setDescription('[ADMIN] IDisallows channel which command is executed to be added.')
				)
				.addSubcommand((subcommand) => subcommand.setName('whitelist').setDescription('[ADMIN] Displays the list of allowed channels.'));
		});
	}
	...
	private async allowPostInSpecificChannel(interaction: Command.ChatInputCommandInteraction) {
		...
		if (allowedChannels.some((c) => c === channel.id)) {
			return await customReply(interaction, `❌ | The allowed channels list already contains \`${channel.name}\`.`);
	...
	private async disallowPostInSpecificChannel(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!allowedChannels.some((c) => c === channel.id)) {
			return await customReply(interaction, `❌ | The allowed channels does not contain \`${channel.name}\`.`);
		} else if (channel.type !== ChannelType.GuildText) {
			return await customReply(interaction, `❌ | Channel \`${channel.name}\` is not a text channel!`);
		}
	...
	private async disallowPostInCurrentChannel(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!allowedChannels.some((c) => c === interaction.channelId)) {
			return await customReply(interaction, `❌ | The allowed channel list does not contain \`${channel?.channel_name}\`.`);
		}
	...
	private async getWhileListChannel(interaction: Command.ChatInputCommandInteraction) {
		...
		if (allowedChannels.length == 0) {
			const embed = new EmbedBuilder().setTitle('Allowed Channels').setDescription('No allowed channels');
		...
		for (let i = 0; i < channels.length; i += channelsPerPage) {
			embedList.push(new EmbedBuilder().setTitle('Allowed Channels').setDescription(channels.slice(i, i + channelsPerPage).join('\n')));
		}
