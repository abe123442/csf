...
export class StandupAdminCommand extends Command {
				...
				.addSubcommand((subcommand) =>
					subcommand
						...
						.addMentionableOption((option) =>
							option.setName('teamrole').setDescription('Mention the team role (@team-role)').setRequired(true)
						)
						.addIntegerOption((option) =>
							option.setName('days').setDescription('Number of days in past to retrieve standups from').setRequired(false)
						)
				)
		);
	}
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator) && !interaction.member.roles.cache.has(TEAM_DIRECTOR_ROLE_ID)) {
		...
		if (interaction.options.getSubcommand() === 'getfullstandups') {
			...
			try {
				...
				thisTeamStandups = thisTeamStandups.filter((st) => Object.keys(roleNames).includes(st.user_id.toString()));
				...
				thisTeamStandups.forEach((standUp) => {
					...
					standupEmbeded.push('**' + `${roleNames[standUp.user_id.toString()]}` + '**' + '\n' + standUp.standup_content);
					sendmsg += '**' + `${roleNames[standUp.user_id.toString()]}` + '**' + '\n' + standUp.standup_content;
					sendmsg += '\n';
				});
				...
				if (thisTeamStandups.length == 0) {
					const embed = new EmbedBuilder()
						.setTitle('Standups (' + role.name + ')')
						.setDescription('No standups recorded\n\n' + '_These users have not done their standup:_\n' + notDoneUsersString);
