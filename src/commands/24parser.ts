...
@ApplyOptions<Command.Options>({
	...
	detailedDescription: {
		usage: '/24parse <expression> <OPTIONAL: target>'
	}
})
	...
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) => {
			builder
				...
				.addStringOption((option) => option.setName('equation').setDescription('Equation for the 24 game').setRequired(true))
				.addNumberOption((option) => option.setName('target').setDescription('Target for your equation').setRequired(false));
		});
	}
