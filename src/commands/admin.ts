...
export class AdminCommand extends Command {
		...
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addSubcommand((subcommand) =>
					subcommand.setName(COMMAND_KICKUNVERIFIED).setDescription('Kicks all unverified users from the server.')
				)
				.addSubcommand((subcommand) =>
					subcommand
						.setName(COMMAND_MIGRATE)
						.setDescription('Migrates a course role to permission overwrites.')
						.addStringOption((option) => option.setName('course').setDescription('Course role to remove').setRequired(true))
				)
				.addSubcommand((subcommand) =>
					subcommand.setName(COMMAND_REMOVECOURSEROLES).setDescription('WARNING: Removes course roles from the server.')
				)
		);
	}
	...
	private async migrate(interaction: Command.ChatInputCommandInteraction) {
		...
		const role = interaction.guild?.roles.cache.find((course_role) => course_role.name.toLowerCase() === course.toLowerCase());

		if (role === undefined) {
			return await interaction.reply('Error: no role exists for course ' + course);
		}

		const channel = interaction.guild?.channels.cache.find((role_channel) => role_channel.name.toLowerCase() === role.name.toLowerCase());
