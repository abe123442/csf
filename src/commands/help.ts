...
export class HelpCommand extends Command {
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addNumberOption((option) => option.setName('page').setDescription('Requested Help Page').setRequired(false))
		);
	}
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		const filter = (resInteraction: ButtonInteraction) => {
			return (resInteraction.customId === prevId || resInteraction.customId === nextId) && resInteraction.user.id === authorId;
		};
